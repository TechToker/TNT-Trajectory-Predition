...
class ArgoverseInMem(InMemoryDataset):
    ...
    def process(self):
        ...
        for raw_path in tqdm(self.raw_paths, desc="Loading Raw Data..."):
            ...
            # statistics
            traj_num = raw_data['all_agents_history'].values[0].shape[0] # amount of trajectory in dataset
        ...
        for ind, raw_path in enumerate(tqdm(self.raw_paths, desc="Transforming the data to GraphData...")):
            ...
            y = self._get_y(raw_data) # get offsets from point to point over the future trajectory. shape=60 (30 time x 2 cord)

            # torch geometric graph data
            # x - node feature matrix. All lines/trajectories with 10 features [num_nodes, num_node_features]
            # edge_index - graph connectivity in coordinate format [2, num_edges]. Connectivity between nodes by indexes
            ...
            graph_input = GraphData(
                ...
                traj_len=torch.tensor([traj_lens[ind]]).int(),            # number of traj polyline on scene
                valid_len=torch.tensor([valid_lens[ind]]).int(),          # number of valid polylines on scene
                ...
                candidate_mask=[],
                candidate=torch.from_numpy(raw_data['target_candidates'].values[0]).float(),
                ...
                orig=torch.from_numpy(raw_data['origin_pos'].values[0]).float().unsqueeze(0),
                rot=torch.from_numpy(raw_data['rotation_matrix'].values[0]).float().unsqueeze(0),
